...
NTSTATUS Vdma8237Out(PDEVICE_CONTEXT ctx, USHORT port, UCHAR value)
{
  ...
  {
    *pg = value;
    It8888Trace(ctx, IT8888_TRACE_VDMA, port, value, 0);
  ...
  if (port <= 0x0F)
  {
    ...
    if (port <= 0x07)
    {
      ...
      {
        if (!s->FlipFlop0)
          s->Ch[ch].BaseCount = (s->Ch[ch].BaseCount & 0xFF00) | value;
        else
        {
          s->Ch[ch].BaseCount =
              (s->Ch[ch].BaseCount & 0x00FF) | ((USHORT)value << 8);
      ...
      {
        if (!s->FlipFlop0)
          s->Ch[ch].BaseAddr = (s->Ch[ch].BaseAddr & 0xFF00) | value;
        else
        {
          s->Ch[ch].BaseAddr =
              (s->Ch[ch].BaseAddr & 0x00FF) | ((USHORT)value << 8);
      ...
      case 0x0A:
      {
        UCHAR c = ChannelFromMaskPort(value);
        s->Ch[c].Masked = MaskBit(value);
        if (s->Ch[c].Masked)
          s->Mask0 |= (1 << c);
        else
          s->Mask0 &= ~(1 << c);
  ...
  if (port >= 0xC0 && port <= 0xDF)
  {
    ...
    else if ((rel % 8) == 2)
    {
      ...
      case 0xD4:
      {
        UCHAR c = 4 + ChannelFromMaskPort(value);
        s->Ch[c].Masked = MaskBit(value);
        if (s->Ch[c].Masked)
          s->Mask1 |= (1 << (c - 4));
        else
...
VOID Vdma8237Snapshot(PDEVICE_CONTEXT ctx, PIT8888_8237_SNAPSHOT snap)
{
  ...
  snap->Command0 = ctx->Vdma.Command0;
  snap->Command1 = ctx->Vdma.Command1;
  snap->Status0 = ctx->Vdma.Status0;
  snap->Status1 = ctx->Vdma.Status1;
  snap->Mask0 = ctx->Vdma.Mask0;
  snap->Mask1 = ctx->Vdma.Mask1;
  snap->FlipFlop0 = ctx->Vdma.FlipFlop0;
  snap->FlipFlop1 = ctx->Vdma.FlipFlop1;
}

NTSTATUS Vdma8237Prepare(PDEVICE_CONTEXT ctx, UCHAR ch,
