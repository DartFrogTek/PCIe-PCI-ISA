...
NTSTATUS It8888EvtDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT DeviceInit)
{
    ...
    WDF_DMA_ENABLER_CONFIG_INIT(&dmacfg, WdfDmaProfilePacket64, 0x1000000); // 16MB max transfer cap for diagnostics
    status = WdfDmaEnablerCreate(device, &dmacfg, WDF_NO_OBJECT_ATTRIBUTES, &ctx->DmaEnabler);
    if (!NT_SUCCESS(status)) {
        // Keep driver usable for cfg/PIO even if DMA enabler fails; dma-alloc will report failure.
        ctx->DmaEnabler = NULL;
        It8888Trace(ctx, IT8888_TRACE_ERROR, 0xD001, (ULONGLONG)status, 0);
    }

    WDF_INTERRUPT_CONFIG_INIT(&icfg, It8888EvtInterruptIsr, It8888EvtInterruptDpc);
    status = WdfInterruptCreate(device, &icfg, WDF_NO_OBJECT_ATTRIBUTES, &ctx->Interrupt);
    if (!NT_SUCCESS(status)) {
        ctx->Interrupt = NULL;
        It8888Trace(ctx, IT8888_TRACE_ERROR, 0xD002, (ULONGLONG)status, 0);
        status = STATUS_SUCCESS; // IRQ can be added later; keep diagnostic driver alive.
    }

    return status;
}

NTSTATUS It8888EvtPrepareHardware(WDFDEVICE Device, WDFCMRESLIST Raw, WDFCMRESLIST Translated)
{
    ...
    status = WdfFdoQueryForInterface(Device, &GUID_BUS_INTERFACE_STANDARD,
        (PINTERFACE)&ctx->BusInterface, sizeof(BUS_INTERFACE_STANDARD), 1, NULL);
    if (NT_SUCCESS(status)) {
    ...
    It8888PciRead(ctx, 0x00, 2, &val); ctx->VendorId = (USHORT)val;
    It8888PciRead(ctx, 0x02, 2, &val); ctx->DeviceId = (USHORT)val;
    It8888PciRead(ctx, 0x08, 1, &val); ctx->RevisionId = (UCHAR)val;
    ctx->Started = TRUE;
    It8888RefreshDdmaBases(ctx);
    It8888EnableCommandBits(ctx);
    It8888Trace(ctx, IT8888_TRACE_INFO, 0x101, ((ULONGLONG)ctx->DeviceId << 16) | ctx->VendorId, ctx->RevisionId);
