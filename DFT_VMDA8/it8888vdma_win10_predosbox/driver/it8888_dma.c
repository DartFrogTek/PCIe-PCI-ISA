...
{
    if (!ctx->DmaEnabler) return STATUS_DEVICE_NOT_READY;
    if (size == 0 || size > 0x1000000) return STATUS_INVALID_PARAMETER;
    It8888DmaFree(ctx);
    NTSTATUS st = WdfCommonBufferCreate(ctx->DmaEnabler, size, WDF_NO_OBJECT_ATTRIBUTES, &ctx->Dma.CommonBuffer);
    if (!NT_SUCCESS(st)) return st;
    ctx->Dma.Va = WdfCommonBufferGetAlignedVirtualAddress(ctx->Dma.CommonBuffer);
    ctx->Dma.Logical = WdfCommonBufferGetAlignedLogicalAddress(ctx->Dma.CommonBuffer);
    ctx->Dma.Size = size;
    ctx->Dma.BufferId = 1;
    RtlZeroMemory(ctx->Dma.Va, size);
    It8888Trace(ctx, IT8888_TRACE_DMA, size, ctx->Dma.Logical.QuadPart, (ULONGLONG)(ULONG_PTR)ctx->Dma.Va);
...
{
    if (!ctx->Dma.CommonBuffer) return STATUS_NOT_FOUND;
