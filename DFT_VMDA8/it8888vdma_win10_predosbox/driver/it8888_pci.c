...
{
    if (width == 1 || width == 2 || width == 4) { *size = width; return STATUS_SUCCESS; }
    return STATUS_INVALID_PARAMETER;
}

NTSTATUS It8888PciRead(PDEVICE_CONTEXT ctx, USHORT offset, UCHAR width, PULONG value)
{
    ULONG size; NTSTATUS st = WidthToSize(width, &size); ULONG v = 0;
    if (!NT_SUCCESS(st)) return st;
    if (!ctx->BusInterfaceValid) return STATUS_DEVICE_NOT_READY;
    ULONG done = ctx->BusInterface.GetBusData(ctx->BusInterface.Context, PCI_WHICHSPACE_CONFIG, &v, offset, size);
    if (done != size) return STATUS_UNSUCCESSFUL;
...
{
    ULONG size; NTSTATUS st = WidthToSize(width, &size);
    if (!NT_SUCCESS(st)) return st;
    if (!ctx->BusInterfaceValid) return STATUS_DEVICE_NOT_READY;
    ULONG v = value;
    ULONG done = ctx->BusInterface.SetBusData(ctx->BusInterface.Context, PCI_WHICHSPACE_CONFIG, &v, offset, size);
    if (done != size) return STATUS_UNSUCCESSFUL;
    It8888Trace(ctx, IT8888_TRACE_CFG_WRITE, offset | ((ULONG)width << 16), value, 0);
    if (offset >= IT8888_CFG_CH01 && offset <= IT8888_CFG_CH67) It8888RefreshDdmaBases(ctx);
    return STATUS_SUCCESS;
}

NTSTATUS It8888EnableCommandBits(PDEVICE_CONTEXT ctx)
{
    ULONG cmd; NTSTATUS st = It8888PciRead(ctx, 0x04, 2, &cmd);
    if (!NT_SUCCESS(st)) return st;
...
NTSTATUS It8888ApplyDefaultInit(PDEVICE_CONTEXT ctx)
{
    ...
    // Recovered from ITEXXX defaults. Odd low bits are decode/control bits; base used by DDMA ops is &~0xF.
    st = It8888PciWrite(ctx, IT8888_CFG_CH01, 4, 0x83918381); if (!NT_SUCCESS(st)) goto out;
    st = It8888PciWrite(ctx, IT8888_CFG_CH23, 4, 0x83B183A1); if (!NT_SUCCESS(st)) goto out;
    st = It8888PciWrite(ctx, IT8888_CFG_CH45, 4, 0x83D30000); if (!NT_SUCCESS(st)) goto out;
    st = It8888PciWrite(ctx, IT8888_CFG_CH67, 4, 0x83F383E3); if (!NT_SUCCESS(st)) goto out;
    st = It8888PciWrite(ctx, IT8888_CFG_50, 4, 0x01FFF023); if (!NT_SUCCESS(st)) goto out;
    st = It8888PciWrite(ctx, IT8888_CFG_54, 4, 0x8C003F3F); if (!NT_SUCCESS(st)) goto out;
...
NTSTATUS It8888GetInfo(PDEVICE_CONTEXT ctx, PIT8888_INFO info)
{
    ...
    It8888PciRead(ctx, 0x04, 4, &v); info->PciCommandStatus = v; info->LastPciStatus = v >> 16;
    ...
    for (int i=0;i<8;i++) info->DdmaBase[i] = ctx->DdmaBase[i];
...
NTSTATUS It8888ClearErrors(PDEVICE_CONTEXT ctx)
{
    ...
    ULONG cs=0; NTSTATUS st = It8888PciRead(ctx, 0x04, 4, &cs);
    if (!NT_SUCCESS(st)) return st;
