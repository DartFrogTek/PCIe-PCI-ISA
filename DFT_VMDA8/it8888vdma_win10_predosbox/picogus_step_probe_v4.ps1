# picogus_step_probe_v4.ps1
# Run from DFT_VMDA8\it8888vdma_win10_predosbox.
#
# Fixes v3 PowerShell 5 / .NET Framework issue:
#   ProcessStartInfo.ArgumentList is not available/reliable there.
#   v4 builds a quoted Arguments string instead.
#
# No 16-bit OUT to odd 0x81D1.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\picogus_step_probe_v4.ps1
#   powershell -ExecutionPolicy Bypass -File .\picogus_step_probe_v4.ps1 -DoSbReset
#   powershell -ExecutionPolicy Bypass -File .\picogus_step_probe_v4.ps1 -DoSbReset -DoIrqTest
...
function Invoke-Ctl {
    ...
    $strArgs = @()
    foreach ($a in $Args) { $strArgs += [string]$a }

    Write-Host ">>> $Ctl $($strArgs -join ' ')"

    $psi = New-Object System.Diagnostics.ProcessStartInfo
    $psi.FileName = $Ctl
    $psi.Arguments = (($strArgs | ForEach-Object { Quote-Arg $_ }) -join " ")
    ...
    if (-not $p.WaitForExit($TimeoutMs)) {
        try { $p.Kill() } catch {}
        Write-Host "TIMEOUT/HANG after $TimeoutMs ms: $($strArgs -join ' ')" -ForegroundColor Red
    ...
    if ($p.ExitCode -ne 0) {
        throw "it8888ctl exit $($p.ExitCode): $($strArgs -join ' ')"
    }
...
Write-Host "============================================================"
Write-Host "PicoGUS staged probe v4"
Write-Host "PowerShell 5 compatible process wrapper; no odd 16-bit OUT."
Write-Host "============================================================"
