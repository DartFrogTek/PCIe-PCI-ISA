	...
	if (NT_SUCCESS (status))
		...
		if (NT_SUCCESS (status))
			...
			ASSERT (gf1_common);
				...
				// Initialize GUS with bridge support for PicoGUS
				status = gf1_common->init_with_bridge_support (resources, g_physical_device, device_object);
			#else
				// Initialize GUS with standard ISA detection
